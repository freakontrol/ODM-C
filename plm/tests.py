...
class PartModelTest(TestCase):
    def setUp(self):
        ...
        self.part = Part()
        self.part.create_new_item(description='Test part', purchase_option=self.purchase_option, category=self.category)
        self.part.save()
        self.assertEqual(self.part.internal_part_number, 'HW-TST001-v01r01')
    def test_new_item_number(self):
        next_part = Part()
        next_part.create_new_item(description='Test part', purchase_option=self.purchase_option, category=self.category)
        next_part.save()
        self.assertEqual(next_part.internal_part_number, 'HW-TST002-v01r01')
    def test_new_variant(self):
        next_part = Part()
        next_part.create_new_variant(description='Test part', purchase_option=self.purchase_option, category=self.category, item_number=1)
        next_part.save()
        self.assertEqual(next_part.internal_part_number, 'HW-TST001-v02r01')
    def test_new_revision(self):
        next_part = Part()
        next_part.create_new_revision(description='Test part', purchase_option=self.purchase_option, category=self.category, item_number=1, variant=1)
        next_part.save()
